...
// BlogSort 값 검증 타입 가드
const isBlogSort = (value: string | null | undefined): value is BlogSort => {
  return Object.values(BlogSort).includes(value as BlogSort);
};
...
export const useBlogs = () => {
  ...
  const sort: BlogSort | undefined = isBlogSort(rawSort) ? rawSort : undefined;
  ...
  return useInfiniteQuery<BlogListResponse, Error, InfiniteData<BlogListResponse>, any, number | undefined>({
...
export const useUpdateBlogFilters = () => {
  ...
  return (newFilters: { size?: number; keyword?: string; sort?: BlogSort }) => {
...
export const useCreateBlog = () => {
  ...
  return useMutation<Blog, Error, BlogRequest, { previousBlogs: BlogListResponse | undefined }>({
    ...
    onMutate: async (newBlog) => {
      ...
      const previousBlogs = queryClient.getQueryData<BlogListResponse>(blogQueryKeys.blog.list({}));
      if (previousBlogs) {
        queryClient.setQueryData<BlogListResponse>(blogQueryKeys.blog.list({}), {
          ...previousBlogs,
          data: {
            ...previousBlogs.data,
            blogs: [
...
export const useDeleteBlog = () => {
  ...
  return useMutation<
    ...
    Error,
    number,
    { previousBlog: Blog | undefined; previousBlogs: BlogListResponse | undefined }
  >({
...
export const useAddBookmark = () => {
  ...
  return useMutation<
    ...
    Error,
    number,
    { previousBlog: Blog | undefined; previousBookmarkedBlogs: BlogListResponse | undefined }
  >({
    mutationFn: (blogId) => blogService.addBookmark(blogId),
    onMutate: async (blogId) => {
      await queryClient.cancelQueries({ queryKey: blogQueryKeys.blog.detail(blogId) });
      const previousBlog = queryClient.getQueryData<Blog>(blogQueryKeys.blog.detail(blogId));
      const previousBookmarkedBlogs = queryClient.getQueryData<BlogListResponse>(blogQueryKeys.blog.bookmarked({}));
      if (previousBlog) {
        queryClient.setQueryData<Blog>(blogQueryKeys.blog.detail(blogId), {
          ...previousBlog,
        });
      ...
      if (previousBookmarkedBlogs && previousBlog) {
        queryClient.setQueryData<BlogListResponse>(blogQueryKeys.blog.bookmarked({}), {
