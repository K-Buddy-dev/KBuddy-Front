...
export const blogService = {
  ...
  createBlog: async (data: BlogRequest): Promise<CommunityPostResponse> => {
    const response = await authClient.post<CommunityPostResponse>('/blog', data);
    return response.data;
  },
  //   // 블로그 수정
  ...
  //     await authClient.post(`/blog/${blogId}/bookmarks`);
  //   },
  //   // 블로그 북마크 삭제
  //   removeBookmark: async (blogId: number): Promise<void> => {
  //     await authClient.delete(`/blog/${blogId}/bookmarks`);
  //   },
  //   // 블로그에 댓글 작성
